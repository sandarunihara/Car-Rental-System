...
const Userdetail = () => {
  ...
  const [user,setuser]=useState({})
    ...
    const userdetails=async()=>{
      if(userrole==='User'){
        const response=await fetch(`http://localhost:8050/api/getcustomer/${userid}`,{
          method:'get',
          headers:{
      ...
      if(userrole==='CarOwner'){
        const response=await fetch(`http://localhost:8050/api/fetchowner/${userid}`,{
          method:'get',
          headers:{
              "content-type": "application/json"
          }
  ...
  const handledelete=async()=>{
    if(userrole==='User'){
      const response=await fetch(`http://localhost:8050/api/deletecustomer/${user._id}`,{
        method:'delete',
        headers:{
            "content-type": "application/json"
        }
    ...
    if(userrole==='CarOwner'){
      const response=await fetch(`http://localhost:8050/api/deleteowner/${user._id}`,{
        method:'delete',
        headers:{
            "content-type": "application/json"
        }
